...
function DonutChart(container_class, data) {
  ...
  this.draw_chart = function () {
    ...
    var svg = d3.select(this.container_class).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(".concat(margin.left, ", ").concat(margin.top, ")")); // Create dummy data
    ...
    var color = d3.scaleOrdinal().domain(data_keys).range(d3.schemeDark2); // Compute the position of each group on the pie:
    ...
    console.log(data_ready); // The arc generator
...
/******/ });
//# sourceMappingURL=d3-charts-e564b50cdbb2d98e19a1.js.map